;; terminal-jnbgo-intro.scm — a faithful port of jnbgo's INTRO scene (scene/intro.go)
;; to the fluxus terminal primitive. Monochrome white-on-black data field that evolves
;; on an 80ms tick (grid/streams/columns/border/binary rows + scanlines/shifts/pulses/
;; bursts), with the dystopian poem revealed as scattered typewriter fragments, fading
;; glitches, a glitching title bar, and a "→ NEXT" hint. Raw terminal output = 1:1
;; (no NTSC — jnbgo emits a plain TUI). Structure mirrors intro.go section for section.
(retained)
(clear)
(hide-editor)
(set-window-size 900 560)

(define W 84)
(define H 30)
(define t (build-terminal W H))
(with-primitive t
  (scale (vector 0.265 0.265 0.265))
  (translate (vector (- (/ (* W 0.5) 2)) (/ (* H 0.9) 2) 0)))
...
(define (build-frame)
  ...
  ;; emit: white on black, positioned row by row
  (define out (open-output-string))
  (write-string (ansi-fg 255 255 255) out)
  (for ([y (in-range H)])
