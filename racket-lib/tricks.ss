...
#lang racket/base
;; fluxus->JUCE port: upstream requires scheme/class (never used here — no class
;; appears in this file) and the scratchpad aggregator "fluxus.ss"; neither is in
;; this library. brew minimal-racket has no `scheme` collection at all, so the
;; class-system path here is racket/class. Require the deps this file actually uses.
(require "fluxus-modules.ss")
(require "building-blocks.ss")
(require "randomness.ss")
;; only-in: poly-tools re-provides engine prims (build-extrusion …) that
;; fluxus-modules already gives us — a plain require collides on them.
(require (only-in "poly-tools.ss" poly-for-each-tri-sample))
