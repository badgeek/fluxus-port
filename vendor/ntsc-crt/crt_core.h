...
/* fluxus->JUCE port: we modulate and demodulate the SAME synthetic frame, so the
 * signal is always perfectly aligned — searching for VSYNC/HSYNC only adds a
 * multi-field lock-on delay (a black screen for the first frames) and can jitter.
 * Turn the search off: the decoder then uses fixed sync positions and locks on
 * frame 1. Set both back to 1 if you ever feed a genuinely noisy/offset signal. */
#define CRT_DO_VSYNC    0  /* look for VSYNC */
#define CRT_DO_HSYNC    0  /* look for HSYNC */
